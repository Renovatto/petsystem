...
class FormatacaoHelper extends AppHelper {
     ...
     * @access public
     */
    public function hora($dataHora = null, $segundos = false, $opcoes = array()) {
        $padrao = array(
            'invalid' => '31/12/1969',
            'userOffset' => null
        );
        ...
        $dataHora = $this->_ajustaDataHora($dataHora);
        if ($segundos) {
            return $this->Time->format('H:i:s', $dataHora, $config['invalid'], $config['userOffset']);
        }
        //return $this->Time->format('H:i', '%B %e, %Y %H:%M %p');
        return $this->Time->format('H:i', $dataHora, $config['invalid'], $config['userOffset']);
    }
